CREATE PROCEDURE sp_folder_permission_grant(
...
sp_folder_permission_grant: BEGIN
  ...
  IF v_folder_exists = 0 THEN
    ROLLBACK; SET p_status_code = 'FOLDER_NOT_FOUND', p_message = 'Folder does not exist or is deleted.'; LEAVE sp_folder_permission_grant;
  END IF;

  SELECT COUNT(*) INTO v_user_exists FROM users WHERE id = p_user_id AND deleted_at IS NULL;
  IF v_user_exists = 0 THEN
    ROLLBACK; SET p_status_code = 'USER_NOT_FOUND', p_message = 'User does not exist.'; LEAVE sp_folder_permission_grant;
  END IF;

  INSERT INTO document_permissions (folder_id, user_id, permission, granted_by)
    VALUES (p_folder_id, p_user_id, p_permission, p_granted_by)
    ON DUPLICATE KEY UPDATE permission = p_permission, granted_by = p_granted_by, granted_at = NOW();

  INSERT INTO audit_logs (user_id, action, entity_type, entity_id, details)
    VALUES (p_granted_by, 'FOLDER_PERMISSION_GRANTED', 'FOLDER', p_folder_id, JSON_OBJECT('granteeUserId', p_user_id, 'permission', p_permission));
  COMMIT;
